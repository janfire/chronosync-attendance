...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function manualStartOnly() {
        console.warn("manualStartOnly called - this should not happen automatically");
    }
    ...
    async function startFaceMode() {
        ...
        fingerprintContainer.classList.remove('visible');
    ...
    async function startFingerMode() {
        selectionScreen.classList.add('hidden');
        fingerprintContainer.classList.add('visible');
    ...
    function handleSuccess(result) {
        ...
        // Play sound
        const audio = new Audio('/sounds/success.mp3'); // Assuming exists or fails silently
        audio.play().catch(e => { });

        showResult('success', result.message.split('!')[0] + '!', result.message.split('!')[1] || `Logged in as ${result.data.user_name}`);

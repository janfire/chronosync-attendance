...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden dark:bg-gray-800 dark:border-gray-700">
        <div class="px-6 py-5 border-b border-gray-100 bg-gray-50 flex items-center gap-3 dark:bg-gray-900 dark:border-gray-700">
            ...
            <div>
                <h3 class="font-bold text-gray-900 dark:text-white">Announcement Details</h3>
                <p class="text-xs text-gray-500 dark:text-gray-400">Compose your message to broadcast to all tenants.</p>
            </div>
        </div>

        <form action="{{ route('superadmin.announcements.store') }}" method="POST" class="p-6 sm:p-8">
            @csrf
            ...
            <div class="mb-6">
                ...
                <div class="relative">
                    <textarea name="message" id="message" rows="4" required class="w-full p-4 bg-white rounded-xl border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm dark:bg-gray-900 dark:border-gray-600 dark:text-white transition-colors" placeholder="What do you want to tell the users? Markdown or HTML is not supported, just plain text."></textarea>
                </div>
                @error('message')
                    <p class="text-red-500 text-xs mt-1"><i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</p>
                @enderror
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                ...
                <div class="bg-gray-50 rounded-xl p-4 border border-gray-100 dark:bg-gray-900/50 dark:border-gray-700">
                    ...
                    <div class="relative">
                        ...
                        <input type="datetime-local" name="expires_at" id="expires_at" value="{{ old('expires_at') }}" class="w-full pl-11 py-3 bg-white rounded-xl border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white transition-colors cursor-pointer">
                    </div>
            ...
            <div class="flex justify-end pt-2 border-t border-gray-100 dark:border-gray-700">
                <button type="submit" class="bg-emerald-600 text-white px-8 py-3 rounded-xl text-sm font-bold shadow-sm shadow-emerald-500/30 hover:bg-emerald-700 hover:shadow-emerald-500/50 transform hover:-translate-y-0.5 transition-all">
                    <i class="fas fa-check-circle mr-2"></i> Create Announcement
                </button>
            </div>
        </form>
    </div>

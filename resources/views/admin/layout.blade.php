        ...
        <aside id="sidebar" class="w-56 hidden lg:flex flex-col bg-white border-r border-gray-200 transition-all duration-200">
            ...
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
                ...
                <div class="mt-6 pt-4 border-t border-gray-200">
                    <a href="{{ route('profile.show') }}" class="flex items-center space-x-3 px-3 py-2 hover:bg-gray-50 rounded-lg transition-colors mb-2">
                    ...
                    <a href="{{ route('profile.show') }}" class="w-full flex items-center space-x-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors mb-2">
                        <i class="fas fa-user-circle"></i>
                        <span>My Profile</span>
                    </a>
                    <form method="POST" action="{{ route('logout') }}" class="mt-1">
                        @csrf
                        <button type="submit" class="w-full flex items-center space-x-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
    ...
    <script>
        $(function() {
            ...
            const updateToggleLabel = () => {
                ...
                $toggle.html(hidden
                    ? '<i class="fas fa-bars"></i><span>Show Sidebar</span>'
                    : '<i class="fas fa-bars"></i><span>Hide Sidebar</span>');
            };

            const sidebarHidden = localStorage.getItem('sidebarHidden') === 'true';
            if (sidebarHidden) {
                $sidebar.addClass('sidebar-closed').removeClass('sidebar-open');
            }
